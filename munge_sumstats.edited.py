...
describe_cname = {
    ...
    'NSTUDY_MIN': 'Minimum number of studies in which the SNP was genotyped.'
}
...
def parse_dat(dat_gen, convert_colname, merge_alleles, log, args):
    ...
    if args.merge_alleles:
        lookup = { s:"" for s in merge_alleles.SNP }
    for block_num, dat in enumerate(dat_gen):
        ...
        if args.merge_alleles:
            old = ii.sum()
            ii = pd.Series([ s in lookup for s in dat.SNP ], dtype=np.bool)
...
def process_n(dat, args, log):
    ...
    if 'N' in dat.columns:
        #print(str(dat.N))
        n_min = args.n_min if args.n_min else dat.N.astype('int').quantile(0.9) / 1.5
        #n_min = args.n_min if args.n_min else int(dat.N[ int(0.9*len(dat.N)) ]) / 1.5
        old = len(dat)
...
def parse_flag_cnames(log, args):
    ...
    cname_options = [
        ...
        [args.nstudy_min, 'NSTUDY_MIN', '--nstudy-min'],
        [args.snp, 'SNP', '--snp'],
        [args.N_col, 'N', '--N'],
